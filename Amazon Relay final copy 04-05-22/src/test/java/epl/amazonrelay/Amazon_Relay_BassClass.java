...
public class Amazon_Relay_BassClass {
	...
	private static Amazon_CredentialsClass property;
	...
	public static void email_Notification(String errorMessage) {
		...
		property = new Amazon_CredentialsClass();
		Properties login_Credentials = property.credentials();
		String subject_title = login_Credentials.getProperty("Subject_Title_Prod").trim();
		String Top_note = login_Credentials.getProperty("Top_note").trim();
		String Bottom_note_1 = login_Credentials.getProperty("Bottom_note_1").trim();
		String Bottom_note_2 = login_Credentials.getProperty("Bottom_note_2").trim();
		...
		try {
			...
			email.setAuthenticator(new DefaultAuthenticator("selva@wowtruck.in", "56*uWG3e"));
			email.setSSLOnConnect(true);
			email.setStartTLSEnabled(true);
			email.setTLS(true);
			email.setFrom("selva@wowtruck.in");
			email.setSubject(subject_title);
			email.setMsg(Top_note + "\n" + "\n" + errorMessage + "\n" + "\n" + Bottom_note_1 + "\n" + Bottom_note_2);
			email.addTo("technology-team@wowtruck.in");
			email.send();
			System.out.println("Email has been sent to technology-team@wowtruck.in from selva@wowtruck.in");
